...
@section('admin')
  ...
  <!-- Content Wrapper. Contains page content -->
	 ...
	 <tr>
	 	<td><img src="{{ (!empty($user->profile_photo_path))? url('upload/user_images/'.$user->profile_photo_path):url('upload\admin_images\No-Image-Placeholder.svg.png') }}" style="width: 50px; height: 50px;"> </td>
		<td>{{ $user->name }}</td>
		<td>{{ $user->email }}</td>
		<td>{{ $user->phone }}</td>
